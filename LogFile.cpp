	...
	if( m_Open ){
		if( m_EditFlag ){
			...
			if( write( m_Handle, &m_fhd, sizeof(m_fhd) ) != sizeof(m_fhd) ){
				ErrorMB("'%s'に書きこみができませんでした.", m_FileName.c_str());
				r = FALSE;
			}
		}
		if( close(m_Handle) ){
...
int CLogFile::Open(LPCSTR pName, BOOL fNew)
{
	...
	while(1){
		...
		if( !(omode & O_CREAT) && (errno == ENOENT) ){	// １回目でファイルが存在しない場合
            ...
            if( n >= 2 ){
			    if( fNew ){
					if( YesNoMB((sys.m_WinFontCharset != SHIFTJIS_CHARSET) ? "Log file (%s) does not exist, create it ?" : "ログファイル(%s)が見つかりません. 新規に作成しますか ?", pName) != IDYES ) return FALSE;
   				}
...
int CLogFile::PutData(SDMMLOG *sp, int n)
{
	...
	if( lseek(m_Handle, (ULONG)FHDOFF + ((ULONG)n * (ULONG)sizeof(SDMMLOG)), SEEK_SET) == -1L ){
		ErrorMB( "'%s'への書きこみに失敗しました.", m_FileName.c_str());
		return FALSE;
	}
