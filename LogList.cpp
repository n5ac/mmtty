...
void __fastcall TLogListDlg::UpdateLogCount(int sw)
{
	...
	if( sw ){
		...
		if(Font->Charset != SHIFTJIS_CHARSET){
			...
			sprintf(bf, "%s - [%u件のデータがあります]", Log.m_FileName.c_str(), Log.GetCount());
		}
		Caption = bf;
	}
	Grid->Col = 0;
...
{
	if( YesNoMB( (Font->Charset != SHIFTJIS_CHARSET)?"Delete Selection.(will not be able to restore)  Are you sure?":"現在選択されている範囲を削除します.削除した内容は復元できません\r\n\r\nよろしおまっか？") == IDYES ){
...
void __fastcall TLogListDlg::KFindTopClick(TObject *Sender)
{
	if( m_FindCall.IsEmpty() || (Sender == KFindTop) ){
		...
		if( InputQuery("MMTTY", (Font->Charset != SHIFTJIS_CHARSET)?"Callsign":"検索するコールサイン", as) == FALSE ) return;	//JA7UDE 0428
		//jstrupr(as.c_str());
		as = as.UpperCase();
		m_FindCall = as;
	}
	...
	if( n >= 0 ){
		...
		WarningMB("'%s'は見つかりませんでした.", m_FindCall.c_str());
	}
}
//---------------------------------------------------------------------------
...
{
	if( YesNoMB( (Font->Charset != SHIFTJIS_CHARSET)?"This job may need long time. Are you sure?":"この処理はメチャンコ時間がかかるかも知れません.\r\n\r\nよろしおまっか？" ) == IDYES ){
		int top = Grid->Selection.Top - 1;
		int end = Grid->Selection.Bottom - 1;
		if( top == end ){
			top = 0;
	...
	switch(r){
		...
		case IDNO:
			if( Font->Charset != SHIFTJIS_CHARSET ){
				...
				r = OkCancelMB("現在MMTTYが保持しているログデータを全て失います.\r\n\r\nほんまにええでっか？");
			}
			switch(r){
				case IDOK:
					Log.DeleteAll();
...
void __fastcall TLogListDlg::SaveMmlogText(LPCSTR pName)
{
	...
	if( fp != NULL ){
		...
		if( fclose(fp) ){
			ErrorMB( Font->Charset != SHIFTJIS_CHARSET ? "Can't save to '%s'" : "'%s'が正しく作成できませんでした.", pName);
		}
	}
	else if( Font->Charset != SHIFTJIS_CHARSET ){
