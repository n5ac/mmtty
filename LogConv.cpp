...
int CLogText::Open(LPCSTR pName)
{
	...
	if( m_fp != NULL ){
		...
		ErrorMB( (sys.m_WinFontCharset!= SHIFTJIS_CHARSET) ? "Can't open '%s'":"'%s'がオープンできません.", pName);
		return FALSE;
	}
}
//---------------------------------------------------------------------------
...
int CLogText::Create(LPCSTR pName)
{
	...
	if( m_fp != NULL ){
		...
		ErrorMB( (sys.m_WinFontCharset!= SHIFTJIS_CHARSET) ? "Can't write to '%s'":"'%s'が作成できません.", pName);
		return FALSE;
	}
}
//---------------------------------------------------------------------------
...
int CLogText::Close(void)
{
	...
	if( m_fp != NULL ){
		...
		m_fp = NULL;
		if( r ){
			ErrorMB((sys.m_WinFontCharset!= SHIFTJIS_CHARSET) ? "Can't close '%s'":"'%s'が正しくクローズできませんでした.", m_FileName.c_str());
		}
		else if( m_Mode ){
			InfoMB((sys.m_WinFontCharset!= SHIFTJIS_CHARSET) ? "Done (write to '%s')":"'%s'への書きこみを終了しました.", m_FileName.c_str());
		}
	}
	return r ? FALSE : TRUE;
}
	...
	for( i = 0; i < TEXTCONVMAX; i++ ){
		if( (!m_rConv[i].Key.IsEmpty())&&(m_rConv[i].Key != "%EOD") ){
			...
			if( ::Text2MMLOG(sp, t, m_rConv[i].Key ) == FALSE ){
				if( !err ){
					ErrorMB((sys.m_WinFontCharset!= SHIFTJIS_CHARSET) ? "An error occurred with conversion type [%s]." : "変換式[%s]でエラーが発生しました.", m_rConv[i].Key.c_str() );
				}
				err++;
			}
		}
...
int MMLOGtoHAMLOG(SDHAMLOG *hp, SDMMLOG *sp, FILE *dbrfp)
{
	...
	if( sp->etime ){
		tim = sp->etime / 30;
		sprintf(bf, "%02u%02u", tim / 60, tim % 30);
...
int CLogADIF::Close(void)
{
	...
	if( m_fp != NULL ){
		...
		m_fp = NULL;
		if( r ){
			ErrorMB((sys.m_WinFontCharset!= SHIFTJIS_CHARSET) ? "Can't write to '%s'":"'%s'が正しくクローズできませんでした.", m_FileName.c_str());
		}
		else if( m_Mode ){
			InfoMB((sys.m_WinFontCharset!= SHIFTJIS_CHARSET) ? "Done (write to '%s')":"'%s'への書きこみを終了しました.", m_FileName.c_str());
		}
...
void CLogADIF::SetData(SDMMLOG *sp, LPCSTR pKey, LPSTR pData)
{
	...
	if( !strcmpi(pKey, "CALL") ){
		...
		if( !sp->band ){
			int i;
			for( i = 1; _mb[i] != NULL; i++ ){
				if( !strcmpi(_mb[i], pData) ) break;
			}
			if( _mb[i] == NULL ) i = 0;
			sp->band = BYTE(i);
		}
...
/*
     KHz  MODE    DATE    UTC  MYCALL        SENT NR    HISCALL       RCVD NR  TXID
------------------------------1234567890123 123 123456 1234567890123 123 123456
QSO: nnnnn RY YYYY-MM-DD HHMM JE1CKA        599 13 DX  NN7NN         599 03 NV 0
1234 12345 12 1234567890 1234 1234567890123 123 12 123 1234567890123 123 12 12 1
*/
